...
class Groups(ApiContainer):
    ...
    def add_member(self, parent_name, *, user_name=None, group_name=None):
        ...
        return self.databricks.api("POST", "/api/2.0/groups/add-member", _data=data)

    def create(self, group_name, *, if_exists="error"):
        data = {"group_name": group_name}
        if if_exists == "ignore":
            try:
                return self.databricks.api("POST", "/api/2.0/groups/create", _data=data)
    ...
    def get_by_id(self, value):
        return self.databricks.api("GET", "/api/2.0/groups/list")["group_names"]
    ...
    def list_members(self, group_name=None):
        return self.databricks.api("GET", "/api/2.0/groups/list-members", _data={"group_name": group_name})["members"]
    ...
    def list_parents(self, *, user_name=None, group_name=None):
        if user_name:
            ...
            raise DatabricksApiException("Must provide user_name or group_name.")
        return self.databricks.api("GET", "/api/2.0/groups/list-parents", _data=data)["group_names"]
    ...
    def remove_member(self, parent_name, *, user_name=None, group_name=None):
        ...
        if user_name:
            ...
            raise DatabricksApiException("Must provide user_name or group_name.")
        return self.databricks.api("POST", "/api/2.0/groups/remove-member", _data=data)

    def delete(self, group_name, *, if_not_exists="error"):
        if if_not_exists == "ignore":
            try:
                return self.databricks.api("POST", "/api/2.0/groups/delete", _data={"group_name": group_name})
            except DatabricksApiException as e:
