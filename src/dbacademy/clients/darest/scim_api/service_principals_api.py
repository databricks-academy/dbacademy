__all__ = ["ScimServicePrincipalsApi"]

from typing import List
from dbacademy.clients.rest.common import ApiClient, ApiContainer


class ScimServicePrincipalsApi(ApiContainer):

    def __init__(self, client: ApiClient):
        from dbacademy.common import validate

        self.__client = validate(client=client).required.as_type(ApiClient)
        self.base_url = f"{self.__client.endpoint}/api/2.0/preview/scim/v2/ServicePrincipals"

    def list(self):
        response = self.__client.api("GET", f"{self.base_url}")
    ...
    def get_by_id(self, service_principle_id: str):
        return self.__client.api("GET", f"{self.base_url}/{service_principle_id}")
    ...
    def create(self, display_name: str, group_ids: List = None, entitlements: List = None):
        ...
        if not self.__client.endpoint.endswith("gcp.databricks.com"):
        ...
        return self.__client.api("POST", f"{self.base_url}", params, _expected=201)
