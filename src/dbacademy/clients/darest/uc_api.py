...
class MetastoresApi(ApiContainer):
    ...
    def get_metastore_by_id(self, object_id: str):
        return self.__client.api("GET", f"{self.base_url}/{object_id}")
    ...
    def delete_metastore_by_id(self, object_id: str):
        return self.__client.api("DELETE", f"{self.base_url}/{object_id}")
...
class WorkspaceApi(ApiContainer):
    def __init__(self, client: ApiClient, workspace_id: str):
        ...
        self.__client = validate(client=client).required.as_type(ApiClient)
        self.base_url = f"{self.__client.endpoint}/api/2.1/unity-catalog/workspaces/{workspace_id}/metastore"
    ...
    def assign_metastore(self, catalog_name, metastore_id) -> dict:
        ...
        return self.__client.api("PUT", self.base_url, payload)
    ...
    def update_assignment(self, default_catalog_name: Optional[str], metastore_id: Optional[str]):
        ...
        return self.__client.api("PATCH", self.base_url, payload)

    def delete_assignment_id(self):
        return self.__client.api("DELETE", self.base_url)
...
class UcApi(ApiContainer):
    ...
    def workspace(self, workspace_id: str) -> WorkspaceApi:
        return WorkspaceApi(self.__client, workspace_id)
