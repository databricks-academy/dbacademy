...
class PipelinesApi(ApiContainer):
    def __init__(self, client: ApiClient):
        from dbacademy.common import validate

        self.__client = validate(client=client).required.as_type(ApiClient)
        self.base_uri = f"{self.__client.endpoint}/api/2.0/pipelines"

    def list(self, max_results: int = 100) -> List[Dict[str, Any]]:
        pipelines = []
        response = self.__client.api("GET", f"{self.base_uri}?max_results={max_results}")
        ...
        while True:
            ...
            if next_page_token is None:
                break
            response = self.__client.api("GET", f"{self.base_uri}?max_results={max_results}&page_token={next_page_token}")

        return pipelines

    # def list_events_by_id(self):
    #     return self.__client.execute_get_json(f"{self.base_uri}/{pipeline_id}/events")

    # def list_events_by_id(self):
    #     return self.__client.execute_get_json(f"{self.base_uri}/{pipeline_id}/events")

    def get_by_id(self, pipeline_id: str) -> Optional[Dict[str, Any]]:
        return self.__client.api("GET", f"{self.base_uri}/{pipeline_id}", _expected=(200, 404))
    ...
    def get_update_by_id(self, pipeline_id: str, update_id: str) -> Optional[Dict[str, Any]]:
        return self.__client.api("GET", f"{self.base_uri}/{pipeline_id}/updates/{update_id}")

    def delete_by_id(self, pipeline_id: str) -> None:
        import time

        self.__client.api("DELETE", f"{self.base_uri}/{pipeline_id}", _expected=(200, 404))
    ...
    def update_from_dict(self, pipeline_id: str, params: Dict[str, Any]) -> Dict[str, Any]:
        return self.__client.api("PUT", f"{self.base_uri}/{pipeline_id}", params)

    def create_from_dict(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self.__client.api("POST", f"{self.base_uri}", params)
    ...
    def start_by_id(self, pipeline_id: str) -> Dict[str, Any]:
        return self.__client.api("POST", f"{self.base_uri}/{pipeline_id}/updates")
