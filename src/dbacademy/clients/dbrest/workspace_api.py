...
class WorkspaceApi(ApiContainer):
    def __init__(self, client: ApiClient):
        ...
        self.base_url = f"{self.__client.endpoint}/api/2.0/workspace"
    ...
    def ls(self, path: str, recursive: bool = False, object_types: List[str] = None) -> Optional[List[Dict[str, Any]]]:
        ...
        if not recursive:
            try:
                results = self.__client.api("GET", f"{self.base_url}/list", _expected=(200, 404), path=path)
    ...
    def mkdirs(self, path: str) -> Dict[str, Any]:
        params = {"path": path}
        return self.__client.api("POST", f"{self.base_url}/mkdirs", params)

    def delete_path(self, path: str) -> Dict[str, Any]:
        payload = {"path": path, "recursive": True}
        expected = [200, 404]
        return self.__client.api("POST", f"{self.base_url}/delete", payload, _expected=expected)
    ...
    def import_html_file(self, html_path: str, content: str, overwrite: bool = True) -> Dict[str, Any]:
        ...
        return self.__client.api("POST", f"{self.base_url}/import", payload)
    ...
    def export_notebook(self, path: str) -> str:
        return self.__client.api("GET", f"{self.base_url}/export",
                                 path=path,
                                 format="SOURCE",
    ...
    def get_status(self, path: str) -> Union[None, dict]:
        return self.__client.api("GET", f"{self.base_url}/get-status", path=path, _expected=[200, 404])
