...
from dbacademy.clients.dbrest.clusters_api.cluster_config import ClusterConfig
...
class ClustersApi(ApiContainer):
    ...
    def list_node_types(self):
        response = self.__client.api("GET", f"{self.base_uri}/list-node-types")
    ...
    def get_by_id(self, cluster_id) -> Optional[Dict[str, Any]]:
        ...
        return self.__client.api("GET", f"{self.base_uri}/get?cluster_id={cluster_id}", _expected=[200, 400])
    ...
    def terminate_by_id(self, cluster_id) -> None:
        cluster_id: str = validate(cluster_id=cluster_id).required.str()
        return self.__client.api("POST", f"{self.base_uri}/delete", cluster_id=cluster_id)
    ...
    def destroy_by_id(self, cluster_id) -> None:
        cluster_id: str = validate(cluster_id=cluster_id).required.str()
        self.__client.api("POST", f"{self.base_uri}/permanent-delete", cluster_id=cluster_id, _expected=[200, 400])
        return None
